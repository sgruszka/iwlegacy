...
il3945_hw_txq_free_tfd(struct il_priv *il, struct il_tx_queue *txq)
{
	...
	struct il3945_tfd *tfd = &tfd_tmp[txq->q.read_ptr];
	struct pci_dev *dev = il->pci_dev;
	int i, num_tbs;

	num_tbs = TFD_CTL_COUNT_GET(le32_to_cpu(tfd->control_flags));

	if (WARN_ON_ONCE(num_tbs > NUM_TFD_CHUNKS))
		return;

	/* Unmap chunks. */
	for (i = 0; i < num_tbs; i++)
		pci_unmap_single(dev,
				 le32_to_cpu(tfd->tbs[i].addr),
